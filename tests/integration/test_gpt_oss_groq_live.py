"""Live GPT-OSS Groq golden smoke — requires GROQ_API_KEY."""
...
from evaluator_gym.eval.sampling import assert_provider_sampling_contract

pytestmark = pytest.mark.live


@pytest.mark.asyncio
async def test_gpt_oss_20b_golden_parse_success_rate(tmp_path):
    if not os.environ.get("GROQ_API_KEY") and not os.environ.get("OPENAI_API_KEY"):
        pytest.skip("Set GROQ_API_KEY for live GPT-OSS smoke")
    ...
    args = parse_args(
        [
            ...
            str(tmp_path),
            "--run-id",
            "gpt-oss-20b-golden-smoke",
        ]
    )
    out_dir = await run_eval_async(args)
    config = json.loads((out_dir / "config.json").read_text())
    entry = get_model("groq/gpt-oss-20b")
    assert_provider_sampling_contract(entry, config["provider_sampling_args"])
    ...
    assert parse_success >= 0.9, f"parse success {parse_success:.1%} below 90% gate"
